...
library(future)
load("work 2021 second half/Combine data together/pass.RData")
load("work 2021 second half/Combine data together/pass_id.RData")
load("work 2021 second half/Add another cores ratio/pass_cores.RData")
load("work 2021 second half/Joint model data anlaysis/mvmm_inits.RData")
source("work 2021 second half/Functions/function.R")
...
ICJM_DA <- list(mcmc = lapply(samples.icjm.psa.cr, function(x) {x$mcmc}),
...
mcmcplot(ICJM_DA$mcmc)

save(ICJM_DA, file = "work 2021 second half/Joint model data anlaysis/ICJM_data_analysis.RData")

summary(as.mcmc.list(lapply(1:3, function(x) {
  as.mcmc(ICJM_DA$mcmc[[x]])
})))

mcmcplot(as.mcmc.list(lapply(1:3, function(x) {
  as.mcmc(samples.icjm.psa.cr[[x]]$mcmc)
})))


# MCMC results ------------------------------------------------------------
mcmc.results <- do.call(rbind, lapply(1:3, function(i) {as.matrix(ICJM_DA$mcmc[[i]])}))
mcmc.results <- exp(mcmc.results[,c("gamma[1]","alpha[1,1]", "alpha[2,1]", "alpha[3,1]",
                                "gamma[2]","alpha[1,2]", "alpha[2,2]", "alpha[3,2]")])
round(colMeans(mcmc.results), 2)
round(sapply(1:8, function(i) {sd(mcmc.results[,i])}), 2)
round(sapply(1:8, function(i) {quantile(mcmc.results[,i], probs = 0.025)}), 2)
round(sapply(1:8, function(i) {quantile(mcmc.results[,i], probs = 0.975)}), 2)

# WAIC --------------------------------------------------------------------

res <- as.matrix(ICJM_DA)
ll <- res[,grep("loglike", colnames(res),)]
waic(ll)$estimates["waic",] # 1772.32169
...
# # not in parallel
# rm(iccsmodel3_jags, samples.iccsmodel3_jags)
# iccsmodel3_jags <- jags.model(file = textConnection(model), data = data,
#                              inits = initials, n.chains = 3)  #
# update(iccsmodel3_jags, n.iter = 3000)
# samples.iccsmodel3_jags <- coda.samples(iccsmodel3_jags, variable.names = c("alpha", "gambh", "Sigmab",
#                                                                      "gamma", "tau", "betaL"),
#                                   n.iter = 5000, thin = 5)
# mcmcplot(samples.iccsmodel3_jags)
# summary(samples.iccsmodel3_jags)
# all <- rbind(samples.iccsmodel3_jags[[1]],samples.iccsmodel3_jags[[2]], samples.iccsmodel3_jags[[3]])
# mcse.mat(all)[,2]/apply(all, 2, sd)
# 
# save(samples.iccsmodel3_jags, file = "work 2021 second half/Joint model selection/interval censoring - competing risk - JAGS/Values + Slope/model3_jags.RData")
