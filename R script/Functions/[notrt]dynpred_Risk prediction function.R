...
csdypred <- function(object = NULL, 
  ...
  loglike <- function(b) {
    n_test/2 * log(tau[i,] / (3*pi))  - 2 * sum(sapply(1:n_test, function(n) {
      ...
      t/2 *(exp(gamma[i,,1] * X) * w %*% sapply(1:15, function(l) {
        exp(T.b[l,] %*% gam_bh[i,,1] + alpha[i,1,1] * (XL.b.now[l,] %*% beta[i,] + ZL.b.now[l,] %*% b) + 
              alpha[i,2,1] * (XL.b.now[l,] %*% beta[i,] + ZL.b.now[l,] %*% b - 
  ...
  hazard.survival <-array(NA, dim = c(iter, 15)) # in two risk version, here is c(iter, 15, 2)
  hazard.survival <- exp(gam_bh[1:iter,,1] %*% t(T.hs[1:15,]) + gamma[1:iter,,1] %*% matrix(rep(X, 15), length(X), 15) +
                                        (beta[1:iter,] %*% t(XL.hs.now[1:15,])  + b[1:iter,] %*% t(ZL.hs.now[1:15,])) * c(alpha[1:iter,1,1])  + 
                                        (beta[1:iter,] %*% t(XL.hs.now[1:15,]) + b[1:iter,] %*% t(ZL.hs.now[1:15,]) - 
                                           beta[1:iter,] %*% t(XL.hs.back[1:15,]) - b[1:iter,] %*% t(ZL.hs.back[1:15,])) * c(alpha[j,2,1]))
  surv <-  exp(- t.start/2 * (hazard.survival %*% w))
  ...
  for (i in 1:length(step)) {
    ...
    hazard.overall.survival <- array(NA, dim = c(iter, 15)) # in two risk version, here is c(iter, 15, 2)
    
    for (j in 1:iter) {
      for (m in 1:15) {
        survival[j,m] <- exp(
          - t.step[m]/2 * (w %*% exp(t(T.s[m,,1:15]) %*% gam_bh[j,,1]  + rep(gamma[j,,1] * X, each = 15) + 
                                          (t(XL.s.now[m,,1:15]) %*% beta[j,] + t(ZL.s.now[m,,1:15]) %*% b[j,]) * alpha[j,1,1] + 
                                          (t(XL.s.now[m,,1:15]) %*% beta[j,] + t(ZL.s.now[m,,1:15]) %*% b[j,] - 
                                             t(XL.s.back[m,,1:15]) %*% beta[j,] - t(ZL.s.back[m,,1:15]) %*% b[j,]) * alpha[j,2,1]))
        )
        
      }
    }
    hazard.overall.survival[1:iter,1:15] <- exp(gam_bh[1:iter,,1] %*% t(T.os[1:15,]) + gamma[1:iter,,1] %*% matrix(rep(X, 15), length(X), 15) + 
                                                 (beta[1:iter,] %*% t(XL.os.now[1:15,])  + b[1:iter,] %*% t(ZL.os.now[1:15,])) * c(alpha[1:iter,1,1]) + 
                                                 (beta[1:iter,] %*% t(XL.os.now[1:15,])+ b[1:iter,] %*% t(ZL.os.now[1:15,]) - 
                                                    beta[1:iter,] %*% t(XL.os.back[1:15,]) - b[1:iter,] %*% t(ZL.os.back[1:15,])) * c(alpha[1:iter,2,1]))
    hazard[1:iter,1:15] <- exp(gam_bh[1:iter,,1] %*% t(T.h[1:15,]) +  gamma[1:iter,,1] %*% matrix(rep(X, 15), length(X), 15) + 
                            c(alpha[1:iter,1,1]) * (beta[1:iter,] %*% t(XL.h.now[1:15,]) + b[1:iter,] %*% t(ZL.h.now[1:15,])) + 
                            c(alpha[1:iter,2,1]) * (beta[1:iter,] %*% t(XL.h.now[1:15,]) + b[1:iter,] %*% t(ZL.h.now[1:15,]) - 
                                                      beta[1:iter,] %*% t(XL.h.back[1:15,])- b[1:iter,] %*% t(ZL.h.back[1:15,])))
    risk.store[1:iter,i] <- (u-t)/2 * ((hazard[1:iter,] * survival[1:iter,]) %*% w) /
      c(surv[1:iter])
    
    overall.surv[1:iter,i] <- exp(- u/2 * (hazard.overall.survival[1:iter,] %*% w))
